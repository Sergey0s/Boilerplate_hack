...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order }) => {
  const [ faster, setFaster ] = useState(JSON.parse(localStorage.getItem('faster')) === false ? false : true);
  console.log(faster)
  ...
  return (
    <div className="Place">
      ...
      <div className="Place__choice">
        ...
        <div className="Place__choice-item">
          <span>Назначить</span>
          <input className= {timeError ? "Time__error" : ''}
                 value={time}
                 onFocus={() => {
                   setFaster(false);
                 }}
                 onChange={event => {
                   setFaster(false);
                   setTimeError(!(/^(0[0-9]|1[0-9]|2[0-3])[:]?[0-5][0-9]$/.test(event.target.value)));
        ...
        <footer className="Place__footer">
            {((!faster && !time && !timeError) || (!faster && timeError) && <div className="Place__order disabled">
                Оплатить {price}
            </div>) || <Link to={`/order/${area.id}/${item.id}`} className="Place__order">
                Оплатить {price}
            </Link>}
        </footer>
    </div>
